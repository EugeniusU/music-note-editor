...
import { VIOLIN_TUNE, NOTE_KEYS, PIANO_OCTAVES } from '@/constants/common';
...
function handleClickGuitarKey(fret: number, string: keyof typeof VIOLIN_TUNE) {
...
function getNoteFromGuitarFret(fret: number, string: keyof typeof VIOLIN_TUNE) {
    const openStringNote = VIOLIN_TUNE[string];
...
const currentSelectedFret = computed(() => {
    if (props.selectedNote) {
        ...
        props.selectedNote.forEach(n => {
            ...
            const f = getGuitarFretsFromNote(key + octave, NOTE_KEYS, VIOLIN_TUNE);
